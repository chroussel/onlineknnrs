...
pub struct GraphiteObserver {
    bucket: AtomicBucket,
    histogram_bucket: Vec<String>
}

impl GraphiteObserver {
    pub fn new(bucket: AtomicBucket, histogram_bucket: Vec<String>) -> GraphiteObserver {
        GraphiteObserver {
            bucket,
            histogram_bucket
        }
...
impl Observer for GraphiteObserver {
    ...
    fn observe_histogram(&mut self, key: Key, values: &[u64]) {
        ...
        self.bucket.gauge(&format!("{}.999", key.name())).value(histo.value_at_quantile(0.990));
